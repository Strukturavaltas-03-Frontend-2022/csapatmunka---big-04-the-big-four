...
export class OrderComponent {
  ...
  CombineData(): void {
    ...
    this.orderList.forEach((item, index) => {
      ...
      let idx = this.customerList.findIndex((element) => element.id == item.customerID)
      if (this.customerList[idx])
        this.combinedList[index].customerID = `${this.customerList[idx].first_name}, ${this.customerList[idx].last_name}`
      else
        this.combinedList[index].customerID = "Unknown"

      idx = this.productsList.findIndex((element) => element.id == item.productID)
      if (this.productsList[idx])
        this.combinedList[index].productID = `${this.productsList[idx].name}, ${this.productsList[idx].type}`
      else
        this.combinedList[index].productID = "Unknown"
    })
